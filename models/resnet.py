...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)#torch.Size([32, 64, 48, 48])
        x = self.layer2(x)#torch.Size([32, 128, 24, 24])
        x = self.layer3(x)#torch.Size([32, 256, 12, 12])
        x = self.layer4(x)#torch.Size([32, 512, 6, 6])

        x = self.avgpool(x)#torch.Size([32, 512, 1, 1])
        x = x.view(x.size(0), -1)
...
if __name__ == "__main__":
    input = torch.randn([32, 3, 192,192])
    model = resnet34(False, num_classes=2, img_size=192)
    output = model(input)
    print(output.size())
